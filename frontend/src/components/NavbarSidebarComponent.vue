<template>
  <div>
    <v-app-bar app class="navbar-container">
      <v-app-bar-nav-icon
        @click="drawer = !drawer"
        id="navbar-menu-icon"
      ></v-app-bar-nav-icon>

      <div class="navbar-left">
        <v-img id="navbar-logo" src="../assets/navbar-logo.png"></v-img>
        <v-img id="navbar-title" src="../assets/navbar-title.png"></v-img>
      </div>

      <div class="navbar-right">
        <v-img src="../assets/navbar-toggle-theme-icon.png"></v-img>
        <v-img src="../assets/navbar-user-icon.png"></v-img>
      </div>
    </v-app-bar>
    <v-navigation-drawer
      v-model="drawer"
      app
      class="sidebar-container sidebar-menu"
    >
      <!-- Sidebar content here -->
      <v-sheet class="mx-auto">
        <div class="sidebar-menu">
          <div id="sidebar-menu-container">
            <div class="sidebar-menu-elements">
              <a
                href="/"
                id="sidebar-menu-home"
                :class="
                  isHomeActive
                    ? 'sidebar-menu-innerdiv active'
                    : 'sidebar-menu-innerdiv'
                "
                @click.prevent="
                  isHomeActive = true;
                  isProjectActive = false;
                  isMembersActive = false;
                  isBugsActive = false;
                "
                ><svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  class="bi bi-house-door"
                  viewBox="0 0 16 16"
                >
                  <path
                    d="M8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4.5a.5.5 0 0 0 .5-.5v-4h2v4a.5.5 0 0 0 .5.5H14a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0 0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293L8.354 1.146ZM2.5 14V7.707l5.5-5.5 5.5 5.5V14H10v-4a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5v4H2.5Z"
                    :fill="isHomeActive ? '#BAA2F9' : '#FFF'"
                  />
                </svg>
                <div>Home</div>
              </a>
            </div>

            <div class="sidebar-menu-elements">
              <a
                href="/"
                id="sidebar-menu-home"
                :class="
                  isProjectActive
                    ? 'sidebar-menu-innerdiv active'
                    : 'sidebar-menu-innerdiv'
                "
                @click.prevent="
                  isProjectActive = true;
                  isHomeActive = false;
                  isMembersActive = false;
                  isBugsActive = false;
                "
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    d="M14 3V5H17.59L7.76 14.83L9.17 16.24L19 6.41V10H21V3M19 19H5V5H12V3H5C3.89 3 3 3.9 3 5V19C3 19.5304 3.21071 20.0391 3.58579 20.4142C3.96086 20.7893 4.46957 21 5 21H19C19.5304 21 20.0391 20.7893 20.4142 20.4142C20.7893 20.0391 21 19.5304 21 19V12H19V19Z"
                    :fill="isProjectActive ? '#BAA2F9' : '#FFF'"
                  />
                </svg>
                <div>Projects</div>
              </a>
            </div>

            <div class="sidebar-menu-elements">
              <a
                href="/"
                :class="
                  isMembersActive
                    ? 'sidebar-menu-innerdiv active'
                    : 'sidebar-menu-innerdiv'
                "
                @click.prevent="
                  isHomeActive = false;
                  isProjectActive = false;
                  isMembersActive = true;
                  isBugsActive = false;
                "
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <circle
                    cx="12"
                    cy="8"
                    r="3"
                    :stroke="isMembersActive ? '#BAA2F9' : '#FFF'"
                    stroke-width="2"
                    stroke-linecap="round"
                  />
                  <path
                    d="M15.2679 8C15.5332 7.54063 15.97 7.20543 16.4824 7.06815C16.9947 6.93086 17.5406 7.00273 18 7.26795C18.4594 7.53317 18.7946 7.97 18.9319 8.48236C19.0691 8.99472 18.9973 9.54063 18.7321 10C18.4668 10.4594 18.03 10.7946 17.5176 10.9319C17.0053 11.0691 16.4594 10.9973 16 10.7321C15.5406 10.4668 15.2054 10.03 15.0681 9.51764C14.9309 9.00528 15.0027 8.45937 15.2679 8L15.2679 8Z"
                    :stroke="isMembersActive ? '#BAA2F9' : '#FFF'"
                    stroke-width="2"
                  />
                  <path
                    d="M5.26795 8C5.53317 7.54063 5.97 7.20543 6.48236 7.06815C6.99472 6.93086 7.54063 7.00273 8 7.26795C8.45937 7.53317 8.79457 7.97 8.93185 8.48236C9.06914 8.99472 8.99727 9.54063 8.73205 10C8.46683 10.4594 8.03 10.7946 7.51764 10.9319C7.00528 11.0691 6.45937 10.9973 6 10.7321C5.54063 10.4668 5.20543 10.03 5.06815 9.51764C4.93086 9.00528 5.00273 8.45937 5.26795 8L5.26795 8Z"
                    :stroke="isMembersActive ? '#BAA2F9' : '#FFF'"
                    stroke-width="2"
                  />
                  <path
                    d="M16.8816 18L15.9012 18.1974L16.0629 19H16.8816V18ZM20.7201 16.9042L21.6626 16.5699L20.7201 16.9042ZM14.7808 14.7105L14.1759 13.9142L13.0193 14.7927L14.2526 15.5597L14.7808 14.7105ZM19.8672 17H16.8816V19H19.8672V17ZM19.7777 17.2384C19.7706 17.2186 19.7641 17.181 19.7725 17.1354C19.7803 17.0921 19.7982 17.0593 19.8151 17.0383C19.8473 16.9982 19.8739 17 19.8672 17V19C21.0132 19 22.1413 17.9194 21.6626 16.5699L19.7777 17.2384ZM17 15C18.6415 15 19.4026 16.1811 19.7777 17.2384L21.6626 16.5699C21.1976 15.2588 19.9485 13 17 13V15ZM15.3856 15.5069C15.7701 15.2148 16.2819 15 17 15V13C15.8381 13 14.9027 13.3622 14.1759 13.9142L15.3856 15.5069ZM14.2526 15.5597C15.2918 16.206 15.727 17.3324 15.9012 18.1974L17.8619 17.8026C17.6439 16.7204 17.0374 14.9364 15.3089 13.8614L14.2526 15.5597Z"
                    :fill="isMembersActive ? '#BAA2F9' : '#FFF'"
                  />
                  <path
                    d="M9.21924 14.7105L9.74736 15.5597L10.9807 14.7927L9.82409 13.9142L9.21924 14.7105ZM3.27986 16.9041L4.22233 17.2384V17.2384L3.27986 16.9041ZM7.11841 18V19H7.93709L8.09873 18.1974L7.11841 18ZM7.00008 15C7.71809 15 8.22992 15.2148 8.61439 15.5069L9.82409 13.9142C9.09729 13.3621 8.16196 13 7.00008 13V15ZM4.22233 17.2384C4.59738 16.1811 5.35849 15 7.00008 15V13C4.05157 13 2.80244 15.2587 2.33739 16.5699L4.22233 17.2384ZM4.13284 17C4.12606 17 4.1527 16.9982 4.18492 17.0383C4.20182 17.0593 4.21967 17.0921 4.22754 17.1354C4.23586 17.181 4.22937 17.2186 4.22233 17.2384L2.33739 16.5699C1.8587 17.9194 2.98683 19 4.13284 19V17ZM7.11841 17H4.13284V19H7.11841V17ZM8.09873 18.1974C8.27295 17.3324 8.7082 16.206 9.74736 15.5597L8.69112 13.8614C6.96263 14.9363 6.35606 16.7203 6.1381 17.8026L8.09873 18.1974Z"
                    :fill="isMembersActive ? '#BAA2F9' : '#FFF'"
                  />
                  <path
                    d="M12 14C15.5715 14 16.5919 16.5512 16.8834 18.0089C16.9917 18.5504 16.5523 19 16 19H8C7.44772 19 7.00829 18.5504 7.11659 18.0089C7.4081 16.5512 8.42846 14 12 14Z"
                    :stroke="isMembersActive ? '#BAA2F9' : '#FFF'"
                    stroke-width="2"
                    stroke-linecap="round"
                  />
                </svg>
                <a href="">Members</a>
              </a>
            </div>
            <div class="sidebar-menu-elements">
              <a
                href="/"
                :class="
                  isBugsActive
                    ? 'sidebar-menu-innerdiv active'
                    : 'sidebar-menu-innerdiv'
                "
                @click.prevent="
                  isHomeActive = false;
                  isProjectActive = false;
                  isMembersActive = false;
                  isBugsActive = true;
                "
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <circle
                    cx="17"
                    cy="8"
                    r="4"
                    :stroke="isBugsActive ? '#BAA2F9' : '#FFF'"
                    stroke-width="2"
                  />
                  <circle
                    cx="12"
                    cy="17"
                    r="3"
                    :stroke="isBugsActive ? '#BAA2F9' : '#FFF'"
                    stroke-width="2"
                  />
                  <circle
                    cx="6.5"
                    cy="9.5"
                    r="2.5"
                    :stroke="isBugsActive ? '#BAA2F9' : '#FFF'"
                    stroke-width="2"
                  />
                </svg>
                <div>My Bugs</div>
              </a>
            </div>
          </div>
        </div>
      </v-sheet>
    </v-navigation-drawer>
  </div>
</template>

<script lang="ts">
export default {
  data() {
    return {
      drawer: false as boolean,
      isHomeActive: false as boolean,
      isProjectActive: false as boolean,
      isMembersActive: false as boolean,
      isBugsActive: false as boolean,
    };
  },
};
</script>

<style scoped>
@import url("https://fonts.googleapis.com/css?family=Roboto");
a {
  text-decoration: none;
  color: inherit;
}
.sidebar-menu-innerdiv {
  display: flex;
  gap: 16px;
  align-items: center;
}
#navbar-menu-icon {
  color: #fbfbfb;
}
.navbar-container {
  background: var(--light-dark, #212121);
  box-shadow: 0px 3px 4px 0px rgba(0, 0, 0, 0.15);
}
.navbar-left,
.navbar-right {
  display: flex;
  align-items: center;
  height: 23px; /* Adjust the height as needed */
}
.navbar-right {
  margin-left: auto;
  width: 72px;
}
.navbar-left {
  width: 175px;
}
#navbar-title {
  width: 75%;
}
#navbar-logo {
  width: 20%;
}
.sidebar-menu {
  border-right: 0.5px solid #a9a9a9;
  background: var(--light-dark, #212121);
  color: #fff;
}
#sidebar-menu-container {
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 16px;
  text-align: center;
}
.sidebar-menu-elements {
  width: 100%;
  padding: 8px 16px;
  font-family: Roboto;
  font-size: 14px;
  font-style: normal;
  font-weight: 500;
  line-height: normal;
}

/* TODO: dynamically toggle active-sidebar-element on sidebar element click  */
.active {
  background-color: #312e37;
  color: var(--light-purple, #baa2f9);
}
</style>
